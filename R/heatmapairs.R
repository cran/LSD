...
pairs(mat,labels=labels,xlim=xlim,ylim=ylim,cex.labels = cexlabels,lower.panel=function(x,y,...){text(sum(xlim)/2,sum(ylim)/2,round(cor(x,y,method=method),digits=2),cex = 3)},main=main,upper.panel=function(x,y,...){heatmapoints(x,y,colpal=colpal,pch=pch,cexplot=cexplot,ncol=ncol,grid=grid,alpha=alpha,add.contour=add.contour,nlevels=nlevels,color.contour=color.contour,greyscale=greyscale,...);if (add.line){abline(h=0,col=color.line,lwd=lwd)};if (add.points){mapoints(x[rownames(mat) %in% group],y[rownames(mat) %in% group],col=color.group,...)}})
}
